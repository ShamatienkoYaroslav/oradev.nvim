...
describe("ora.result.query", function()
  ...
  it("has type, label, icon, and render fields", function()
    ...
    assert.equals("Results", output.label)
...
describe("ora.result.error", function()
  ...
  describe("create()", function()
    ...
    it("has error output type fields", function()
      ...
      assert.equals("Errors", output.label)
      assert.is_string(output.icon)
      assert.is_string(output.icon_hl)
      assert.is_function(output.render)
    end)
  ...
  describe("compile", function()
    ...
    it("shows success for clean output", function()
      ...
      assert.equals("Results", output.label)
      local combined = table.concat(output.lines, "\n")
      assert.matches("successfully", combined)
    ...
    it("has correct output type fields for success", function()
      ...
      assert.is_string(output.icon)
      assert.is_string(output.icon_hl)
      assert.equals("Type", output.icon_hl)
    end)
    ...
    it("shows dbms_output lines in success output", function()
      local raw = "hello\nworld\nPL/SQL procedure successfully completed."
      local output = make_compile(raw)
      local combined = table.concat(output.lines, "\n")
      assert.matches("Output:", combined)
      assert.matches("hello", combined)
      assert.matches("world", combined)
    end)

    it("strips noise from dbms_output", function()
      local raw = "hello\nPL/SQL procedure successfully completed.\nno errors"
      local output = make_compile(raw)
      local combined = table.concat(output.lines, "\n")
      assert.matches("hello", combined)
      assert.is_nil(combined:match("PL/SQL procedure"))
      assert.is_nil(combined:match("no errors"))
    end)
  end)
